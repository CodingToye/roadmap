...
const Fundamentals = () => {
  const [fundamentals, setFundamentals] = useState<Section[]>([]);
  ...
  useEffect(() => {
    ...
    socket.on("updateFundamentals", (newFundamentals) => {
      console.log("Received data:", newFundamentals);
    ...
    return () => {
      socket.off("updateFundamentals");
      socket.close();
    };
  }, []);

  const toggleBlocks = (sectionIndex: number) => {
    setFundamentals((prevSections) =>
        ...
        {fundamentals.map((fundamental, fundamentalIndex) => (
          ...
          >
            <header className="flex items-center">
              ...
              <div
                className="pl-4"
                onClick={() => toggleBlocks(fundamentalIndex)}
              >
